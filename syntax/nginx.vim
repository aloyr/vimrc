...
"   " Generates simple circle images with the colors and size specified in the
"   URL.
...
"   " Brings 'echo', 'sleep', 'time', 'exec' and more shell-style goodies to
"   Nginx config file.
...
"   requests passed to the upstream.
...
"   <https://connectical.com/projects/ngx-fancyindex/wiki>
...
"   <http://wiki.nginx.org/NginxHttp3rdPartyGeoIPModule>
...
"   <http://wiki.github.com/kung-fu-tzu/ngx_http_js_module>
...
"   " Implements a MogileFS client, provides a replace to the Perlbal reverse
"   proxy of the original MogileFS.
...
"   " Will seek to a certain time within H.264/MP4 files when provided with a
...
"   " Easy and robust deployment of Ruby on Rails application on Apache and
"   Nginx webservers.
...
"   " A filter module which can do both regular expression and fixed string
...
"   " Module providing nginx with API to communicate with supervisord and
...
"   " Parses multipart/form-data allowing arbitrary handling of uploaded
"   files.
...
"   " Sends an incoming request to the least-busy backend server, rather than
...
"   " Provides simple upstream load distribution by hashing a configurable
"   variable.
...
"   " Allows Nginx to interact with uWSGI processes and control what
"   parameters are passed to the process.
